...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { label = 'API key', actor = 'operator', reason } = await req.json().catch(() => ({}));
  ...
  await prisma.auditEvent.create({
    data: {
      ...
      actor,
      metadata: JSON.stringify({ prefix, ...(reason ? { reason } : {}) }),
    },
  });
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { key_id, actor = 'operator', reason } = await req.json().catch(() => ({}));
  ...
  await prisma.auditEvent.create({
    data: {
      ...
      actor,
      metadata: JSON.stringify({ key_id, ...(reason ? { reason } : {}) }),
    },
  });
